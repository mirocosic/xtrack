...
export default props => {
  ...
  const loadBackup = async id => {
    ...
    Alert.alert(__("Are you sure?"), __("Do you want to restore this backup? Make sure you have the current state backed up or it will be lost."), [
      {
        text: __("Cancel"),
        style: "cancel",
      },
      {
        ...
        onPress: () => {
          props.restoreBackup(get(backupToRestore, "data"))
        },
      },
    ])
  }
  ...
  const confirmDeleteBackup = id => {
    Alert.alert(__("Are you sure?"), __("Do you want to delete this backup? This action cannot be undone."), [
      {
        text: __("Cancel"),
        style: "cancel",
      },
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 50 }}>
        {backups.map(item => (
          <Swipeable key={item.id} renderRightActions={() => renderActions(item.id)} containerStyle={styles.swiperWrap}>
            <RectButton onPress={() => loadBackup(item.id)} activeOpacity={darkMode ? 0.5 : 0.1} style={[styles.wrap, darkMode && styles.wrapDark]} rippleColor={darkMode ? palette.darkGray : palette.lightBlue}>
