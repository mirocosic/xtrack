...
class Reports extends Component {
  static navigationOptions = ({ navigation }) => ({
    ...
    tabBarIcon: () => <Icon style={{ backgroundColor: palette.blue, height: 50, width: 50, bottom: 10, borderRadius: 50 }} textStyle={{ fontSize: 30, color: "white" }} type="plus" />,
  })
  ...
  render() {
    ...
    const { accounts, darkMode, expenses, income, total, navigation, clearSelectedCategory, clearTransactionForm, baseCurrency } = this.props
    ...
    return (
      <Screen>
        ...
        <ScrollView scrollEventThrottle={16} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: height } } }])}>
          {accounts.map(account => (
            <View key={account.id} style={[styles.accountCard, darkMode && styles.accountCardDark]}>
              <Title>{account.name}</Title>
              <View style={styles.accountDetails}>
                <Copy>
                  {__("Expenses")}: {formatCurrency(this.calcExpenses(account), baseCurrency)}
                </Copy>
                <Copy>
                  {__("Income")}: {formatCurrency(this.calcIncome(account), baseCurrency)}
                </Copy>
                <Copy>
                  {__("Total")}: {formatCurrency(this.calcTotal(account), baseCurrency)}
                </Copy>
              </View>
            </View>
          ))}
          ...
          <View style={[styles.accountCard, darkMode && styles.accountCardDark]}>
            ...
            <View style={styles.accountDetails}>
              <Copy>
                {__("Expenses")}: {formatCurrency(expenses, baseCurrency)}
              </Copy>
              <Copy>
                {__("Income")}: {formatCurrency(income, baseCurrency)}
              </Copy>
              <Copy>
                {__("Total")}: {formatCurrency(total, baseCurrency)}
              </Copy>
            </View>
          </View>
        </ScrollView>
