...
export default props => {
  ...
  const addViewItems = () => {
    setItems(items => {
      ...
      return [...items, { id: lastId + 1 }, { id: lastId + 2 }, { id: lastId + 3 }]
    })
  }
  ...
  const renderItem = ({ item }) => {
    ...
    const filterTransactions = type =>
      filterByMonth(
        transactions.filter(t => t.type === type && !t.isTransfer),
        month.toDate(),
      )
    ...
    return (
      <Animated.ScrollView style={[styles.monthContainer]}>
        <View style={[styles.inlineBetween, { marginTop: 15, marginBottom: 15 }]}>
          {item.id > 0 ? (
            <TouchableOpacity onPress={() => flatListRef.current.scrollToIndex({ index: currentMonthIndex, animated: true })} hitSlop={{ top: 10, right: 10, bottom: 10, left: 10 }}>
  ...
  return (
    <Screen>
      <Animated.FlatList
        ...
        getItemLayout={(items, index) => ({ length: WIDTH, offset: WIDTH * index, index })}
