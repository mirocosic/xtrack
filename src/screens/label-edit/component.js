...
export default ({ navigation, route: { params }, edit, add, deleteLabel }) => {
  ...
  return (
    <Screen>
      ...
      <ScrollView scrollEnabled={false} contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 20, flex: 1, justifyContent: "space-between" }}>
        <View>
          <View style={styles.inputContainer}>
            <Copy>Name</Copy>
            <TextInput ref={inputRef} autoFocus={!label.id} style={[styles.input, darkMode && styles.inputDark]} onChangeText={text => setLabel({ ...label, name: text })} placeholder="tag name" placeholderTextColor="gray" value={label.name} />
          </View>
      ...
      <Modalize adjustToContentHeight modalStyle={[styles.modal, darkMode && styles.modalDark]} ref={colorModal}>
