...
export default React.forwardRef((props, ref) => {
  const { labels, onSelect, navigation } = props
  ...
  return (
    <Modalize onOpen={() => Keyboard.dismiss()} adjustToContentHeight modalStyle={[styles.modal, useDarkTheme() && styles.modalDark]} ref={ref}>
      <ScrollView style={{ minHeight: 200, maxHeight: 400, padding: 10 }} contentContainerStyle={{ paddingBottom: insets.bottom }}>
        {labels.map(label => (
          <TouchableOpacity key={label.id} onPress={() => onSelect(label)}>
        ...
        <TouchableOpacity style={[styles.inline, { justifyContent: "flex-start", paddingLeft: 5 }]} onPress={() => navigation.navigate("LabelEdit", {})}>
