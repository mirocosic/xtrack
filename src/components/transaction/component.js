      ...
      <RectButton
        ...
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <View style={styles.inline}>
            ...
            {transaction.recurring && <Icon type="sync" textStyle={{ color: "gray", fontSize: 14 }} style={{ width: 15, height: 15, marginLeft: 10 }} />}
          </View>
          <Copy>{transaction.note}</Copy>
          <View style={styles.labels}>{transaction.labels && transaction.labels.map(label => <Label key={label.uuid} label={label} style={{ marginLeft: 5, paddingRight: 10, marginBottom: 5 }} />)}</View>
        </View>
      </RectButton>
    </Swipeable>
  )
